...
module.exports.execute = async (client, message, args) => {
    ...
    let embed = new MessageEmbed().setFooter(`Diego Was Here!`).setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor("#000000")
    let user = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
    if(!user) return message.channel.send(embed.setDescription(`Bir Kullanıcı Etiketleyiniz. \`${config.prefix}yetkili @Diego/ID yükselt veya ${config.prefix}yetkili @Diego/ID düşür\` `)).catch(e => { })
    let yd = args[1];
    if(!yd) return message.channel.send(`Bir veri belirtiniz \`${config.prefix}yetkili @Diego/ID yükselt veya ${config.prefix}yetkili @Diego/ID düşür\` `).catch(e => { })
    if(yd === "yükselt") {
      let yetkiNumber;
...
msj.awaitReactions(kabul, {max: 1, time: 50000, error: ['time']}).then(async c => {
 ...
 exports.conf = {
    command: "yetkiver2", // Asıl komutumuz
